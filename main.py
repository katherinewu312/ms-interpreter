...
class Def(Node):
    ...
    def anlz_procs(self):
        if self.name not in proc_env:
            #add procedure definition to proc_env
            #associate parameters and body with procedure name
...
class Call(Node):
    ...
    def exec(self, local_var_env, is_global):
        if self.name in proc_env:
            ...
            #number of args in call must equal number of parameters defined in proc_env
            ...
            #exec procedure body, with the local variables as the parameters
